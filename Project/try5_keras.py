...
def make_timeseries_regressor(window_size, filter_length, nb_input_series=1, nb_outputs=1, nb_filter=4):
    ...
    model = Sequential((
        ...
        Conv1D(filters=nb_filter/2, kernel_size=filter_length, padding='same', activation='relu'),
...
def evaluate_timeseries_with_label(timeseries, labels, window_size):
    ...
    #y = np.atleast_2d(labels)[:123].reshape(546,123) # 546,123, here 123 should be same as Dense out
...
def save(X, y, predicted):
    ...
    df.to_pickle('try5_keras.out')


def main():
    _data, _label = gen_data()
