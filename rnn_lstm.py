...
#Yu: this will learn how many 1 in a binary string of length 20
import numpy as np
import random
...
# train_input has 2**x samples and each is of 20 sequence_length, meaning it's 20 Time teps.
...
#Yu: but why not use val.get_shape()[1]
val = tf.transpose(val, [1, 0, 2])
last = tf.gather(val, int(val.get_shape()[0]) - 1)

# This part is a logictic regression
...
for i in range(epoch):
    ...
    if (i%10==0):
        err = sess.run(error, {data: inp, target: out}) 
        print("Epoch " + str(i) + ", Minibatch Error= " + \
                "{:.4f}".format(err) )
        if err <1e-6:
            print("converged with 1e-6 accuracy tolerance")
            break
